...
List of backlink files. Note: This property is performance heavy. When possible, reverse the lookup and use `file.links`. Does not automatically refresh results when the vault is changed.
...
List of all embeds in the note
...
File object, only usable in specific functions
...
List of all internal links in the note, including frontmatter
...
All properties on the file. Note: Does not automatically refresh results when the vault is changed.
...
List of all tags in the file content and frontmatter
