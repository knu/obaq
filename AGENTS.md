...
- `src/` contains the TypeScript source for the CLI and query engine.  Tests live alongside code as `*.test.ts`.
...
- `scripts/` contains maintenance tasks (for example, fetching reference docs).
- `docs/` is generated by `npm run fetch:docs`, ignored by git, and must not be committed due to copyright.

## Compatibility Goal
- This tool aims to process Base queries with compatibility against the Obsidian Bases specifications.  When implementing features, prioritize matching official behavior as closely as possible.
...
- `npm run fetch:docs`: download Obsidian reference pages into `docs/` (ignored).
